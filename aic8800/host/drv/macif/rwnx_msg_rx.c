                                                    ...
                                                    struct e2a_msg *msg)
{
    ...
    struct mm_remain_on_channel_exp_ind *param = (struct mm_remain_on_channel_exp_ind *)msg->param;
    //if (mac_vif_index != param->vif_index) {
    //    printk("%s %d %d\n", __func__, mac_vif_index, param->vif_index);
    //    return -1;
    //}
    struct fhost_vif_tag *fhost_vif = fhost_from_mac_vif(param->vif_index);
    ...
    event.hdr.id = CFGRWNX_CANCEL_REMAIN_ON_CHANNEL_EVENT;
                                                ...
                                                struct e2a_msg *msg)
{
    ...
    if(rwnx_radar_detection_is_enable(&rwnx_hw->radar, pulses->idx)) {
        ...
        extern rtos_semaphore detection_work_sema;
        rtos_semaphore_signal(detection_work_sema, 0);
    } else
        RWNX_DBG("not enable\n");
    return 0;
}
                                          ...
                                          struct e2a_msg *msg)
{
    ...
    if ((mac_vif_index >= NX_VIRT_DEV_MAX)|| (!fhost_env.mac2fhost_vif[ind->vif_index])) {
                                         ...
                                         struct e2a_msg *msg)
{
    ...
    //aic_dbg("ind->vif_index %d\r\n", ind->vif_index);
    vif = fhost_from_mac_vif(ind->vif_index);
    //aic_dbg("vif %x\r\n", vif);
...
int rwnx_force_scan_stop(void)
{
    ...
    struct fhost_vif_tag *fhost_vif = fhost_from_mac_vif(mac_vif_index);
    if (!fhost_vif) {
        aic_dbg("%s fhost_vif is null!\r\n", __func__);
        return -1;
    }
    ...
    return fhost_cntrl_cfgrwnx_event_send(&resp.hdr, fhost_vif->scan_sock);
}
                                          ...
                                          struct e2a_msg *msg)
{
    ...
    if ((cfm->vif_idx >= NX_VIRT_DEV_MAX) || (!fhost_env.mac2fhost_vif[cfm->vif_idx])) {
        aic_dbg("%s %d\r\n", __func__, cfm->vif_idx);
        return -1;
    }
                                           ...
                                           struct e2a_msg *msg)
{
    ...
    if ((ind->inst_nbr >= NX_VIRT_DEV_MAX) || (!fhost_env.mac2fhost_vif[ind->inst_nbr])) {
        aic_dbg("%s %d\r\n", __func__, ind->inst_nbr);
        return -1;
    }
                                         ...
                                         struct e2a_msg *msg)
{
    ...
    if ((param->vif_idx >= NX_VIRT_DEV_MAX) || (!fhost_env.mac2fhost_vif[param->vif_idx])) {
        aic_dbg("%s %d\r\n", __func__, param->vif_idx);
        return -1;
    }
    ...
    if (param->status_code == MAC_ST_SUCCESSFUL)
    {
        ...
        fhost_vif->mac_vif->chan.band = param->band;
        fhost_vif->mac_vif->chan.prim20_freq = param->center_freq;
        fhost_vif->mac_vif->chan.center1_freq = param->center_freq1;
        fhost_vif->mac_vif->chan.type = param->width;
...
static msg_cb_fct scan_hdlrs[MSG_I(SCANU_MAX)] = {
    ...
    //[MSG_I(SCANU_START_CFM_ADDTIONAL)] = rwnx_rx_scanu_started_ind,
};
