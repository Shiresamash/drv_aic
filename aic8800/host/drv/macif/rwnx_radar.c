...
static rtos_mutex pool_lock;

static void pool_register_ref(void)
{
	rtos_mutex_lock(pool_lock, -1);
	singleton_pool_references++;
	rtos_mutex_unlock(pool_lock);
}
...
struct pri_sequence *pde_short_check_detection(struct pri_detector *pde)
{
	...
	list_for_each_entry_safe(ps, ps, &pde->sequences, head) {
		/*
...
{
	struct pri_detector *pde, *pde0, *cur = NULL;
	list_for_each_entry_safe(pde, pde0, &dpd->detectors[radar_type], head) {
		if (pde->freq == freq) {
			if (pde->count)
				return pde;
			else
										   ...
										   u16 freq, u16 pri, u16 len, u32 now)
{
	...
	if (pri == 0) {
		...
		dpd->last_pulse_ts += 1000 * (delta_jiffie);
...
dfs_pattern_detector_init(enum nl80211_dfs_regions region, u8 chain)
{
	...
	AIC_LOG_PRINTF("%s %d\n", __func__, __LINE__);
	dpd = rtos_malloc(sizeof(*dpd) + max_radar_type * sizeof(dpd->detectors[0]));
	if (dpd == NULL)
		return NULL;
...
static u16 rwnx_radar_get_center_freq(struct rwnx_hw *rwnx_hw, u8 chain)
{
	...
	if (chain == RWNX_RADAR_RIU) {
		...
		if (!rwnx_chanctx_valid(rwnx_hw, rwnx_hw->cur_chanctx)) {
			aic_dbg("Radar pulse without channel information");
		} else
...
static void rwnx_radar_detected(struct rwnx_hw *rwnx_hw)
{
	...
	if (!rwnx_chanctx_valid(rwnx_hw, rwnx_hw->cur_chanctx)) {
		aic_dbg( "Radar detected without channel information");
		return;
	}
...
static void rwnx_radar_process_pulse(void)
{
	struct rwnx_radar *radar = &g_rwnx_hw->radar;//container_of(ws, struct rwnx_radar,
								//			detection_work);
	...
	u32 now = rt_tick_get(); /* would be better to store jiffies value in IT handler */

	/* recopy pulses locally to avoid too long spin_lock */
	rtos_mutex_lock(radar->lock, -1);
	...
	for (chain = RWNX_RADAR_RIU; chain < RWNX_RADAR_LAST; chain++) {
		...
		for (i = 0; i < pulses_count[chain] ; i++) {
			...
			if (dfs_pattern_detector_add_pulse(radar->dpd[chain], chain,
				...
				radar->detected[chain].time[idx] = rtos_now(0)/1000;
...
#ifdef CONFIG_RWNX_FULLMAC
extern int drv_link_send_sock;
struct aic_80211_chan_def g_chandef;
int rwnx_send_apm_stop_cac_req(struct rwnx_hw *rwnx_hw, uint8_t vif_index);
static void rwnx_radar_cac_work(void *param)
{
	struct cfgrwnx_dfs_event event;
	struct aic_80211_chan_def *chandef = (struct aic_80211_chan_def *)param;
	aic_dbg("%s enter\n", __func__);
	aic_dbg("%s  %x center_freq %d\n", __func__, chandef, chandef->chan->center_freq);
	//struct delayed_work *dw = container_of(ws, struct delayed_work, work);
	struct rwnx_radar *radar = &g_rwnx_hw->radar;//container_of(dw, struct rwnx_radar, cac_work);
	struct rwnx_hw *rwnx_hw = container_of(radar, struct rwnx_hw, radar);
	//struct rwnx_chanctx *ctxt;

	if (radar->cac_vif == NULL) {
		aic_dbg( "CAC finished but no vif set");
		return;
	}
    event.hdr.id = CFGRWNX_DFS_CAC_FINISHED_EVENT;
    event.hdr.len = sizeof(event);
	event.center_freq = chandef->chan->center_freq;
	event.ht_enabled = 1;
	event.chan_offset = (chandef->center_freq1 - chandef->chan->center_freq);
	event.chan_width = chandef->width;
	event.cf1 = chandef->center_freq1;
	event.cf2 = chandef->center_freq2;
	//event.duration = roc_elem->duration;
	if ((drv_link_send_sock == -1) || (fhost_cntrl_cfgrwnx_event_send(&event.hdr, drv_link_send_sock)))
		aic_dbg("%s event send fail\n", __func__);
	...
	rwnx_send_apm_stop_cac_req(rwnx_hw, 1);
...
rtos_semaphore detection_work_sema = NULL;
rtos_task_handle detection_work_task;
static void detection_work(void *param)
{
    int ret = 0;

    while (1) {
        ret = rtos_semaphore_wait(detection_work_sema, -1);
        if (ret == 0) {
			rwnx_radar_process_pulse();
		}
    }
}
...
{
	int ret = 0;
	rtos_mutex_create(&radar->lock, "radar_lock");
	rtos_mutex_create(&pool_lock, "pool_lock");
	...
	rtos_semaphore_create(&detection_work_sema, "detection_work_sema", 1, 0);
	ret = rtos_task_create(detection_work, "detection_work", 0, 2048, NULL, RADAR_DETECTION_TASK_PRIORITY, &detection_work_task);
    if (ret != 0)
        AIC_LOG_PRINTF("detection_work_create failed\n");
#ifdef CONFIG_RWNX_FULLMAC
...
{
	aic_dbg("%s cac_time_ms %d\n", __func__, cac_time_ms);
	aic_dbg("%s  %x center_freq %d\n", __func__, chandef, chandef->chan->center_freq);
	if (radar->cac_vif != NULL)
		aic_dbg("CAC already in progress");
	radar->cac_vif = vif;
	memcpy(&g_chandef, chandef, sizeof(*chandef));
	// schedule_delayed_work(&radar->cac_work, msecs_to_jiffies(cac_time_ms));
	if (!cac_work_timer) {
		rtos_timer_create("cac_work_timer", &cac_work_timer,
							cac_time_ms/10, 0, &g_chandef, rwnx_radar_cac_work);
	}
	rtos_timer_start(cac_work_timer, 0, 0);
}
void rwnx_radar_cancel_cac(struct rwnx_radar *radar)
{
	...
	if (RT_EOK == rtos_timer_stop(cac_work_timer, 0)) {
		struct rwnx_chanctx *ctxt;
		ctxt = &rwnx_hw->chanctx_table[radar->cac_vif->chan_index];
		rwnx_send_apm_stop_cac_req(rwnx_hw, 1);
		
		event.hdr.id = CFGRWNX_DFS_CAC_ABORTED_EVENT;
		event.hdr.len = sizeof(event);
		event.center_freq = g_chandef.chan->center_freq;
		event.ht_enabled = 1;
		event.chan_offset = (g_chandef.center_freq1 - g_chandef.chan->center_freq);
		event.chan_width = g_chandef.width;
		event.cf1 = g_chandef.center_freq1;
		event.cf2 = g_chandef.center_freq2;
								 ...
								 struct pri_detector *pde)
{
	...
	if (buf == NULL) {
		int nb_seq = 1;
		list_for_each_entry_safe(seq, seq0, &pde->sequences, head) {
			nb_seq++;
		}
	...
	res = snprintf(buf, len, freq_info, pde->freq);
	write += res;
	len -= res;

	res = snprintf(&buf[write], len, "%s", seq_info);
	write += res;
	len -= res;

	list_for_each_entry_safe(seq, seq0, &pde->sequences, head) {
		res = snprintf(&buf[write], len, " %6.d |   %2.d  |    %.2d \n",
									 ...
									 struct rwnx_radar *radar, u8 chain)
{
	...
	/* */
	for (i = 0; i < dpd->num_radar_types; i++) {
		res = snprintf(&buf[write], len, info, i);
...
{
    time_t secs = (time_t)(totalsecs + offset);
    struct tm *tm_ptr = localtime(&secs);
