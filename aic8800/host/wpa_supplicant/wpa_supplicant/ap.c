...
struct wpabuf* aic_build_vendor_specific_ie(void)
{
	struct wpabuf *ie = NULL;
	//u8 *len;
	wpa_printf(MSG_ERROR, "AIC: Building Vendor Specific IE[%d]\n", 
	                                                            strlen(custom_vendor_ie));
	 ie = wpabuf_parse_bin(custom_vendor_ie);//wpabuf_alloc(strlen(custom_vendor_ie)/2);
	if (ie == NULL)
		return NULL;
	//wpabuf_put_u8(ie, WLAN_EID_VENDOR_SPECIFIC);
	//len = wpabuf_put(ie, 1);
	//wpabuf_put_be24(ie, AIC_DEV_OUI);
	//u8 vendor_data_element[5] = {0x00, 0x00, 0x02, 0x00, 0x22};
	//wpabuf_put_data(ie, hexstr2bin(custom_vendor_ie), strlen(custom_vendor_ie)/2);
	//*len = wpabuf_len(ie) - 2;
...
#ifdef CONFIG_VENDOR_IE
            if (custom_vendor_ie)
                hapd_iface->bss[0]->conf->assocresp_elements = aic_build_vendor_specific_ie();
#endif
