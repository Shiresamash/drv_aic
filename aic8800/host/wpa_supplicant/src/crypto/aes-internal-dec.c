...
#include "crypto.h"
#include "aes_i.h"
#ifdef CONFIG_AES_ALT
#include "ce_api.h"
#endif /* CONFIG_AES_ALT */

#ifndef CONFIG_AES_ALT
/**
...
int aes_decrypt(void *ctx, const u8 *crypt, u8 *plain)
{
	...
	for(int i = 0; i < 8; i++)
		dma_cypt_obj.aes_key[i] = k[i];

	dma_cypt_init(&dma_cypt_obj);
	aes_obj.aes_data = (uint32_t *)crypt;
	aes_obj.length_byte = AES_BLOCK_SIZE;

	aes_calc(&dma_cypt_obj, &aes_obj, (uint32_t *)plain);
