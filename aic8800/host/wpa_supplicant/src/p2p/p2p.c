...
{
#ifndef PLATFORM_ALLWIN_RT_THREAD
...
#else
	if (p2p->cfg->ssid_postfix && p2p->cfg->ssid_postfix_len > 0) {
		os_memcpy(ssid, p2p->cfg->ssid_postfix, p2p->cfg->ssid_postfix_len);
		*ssid_len =  p2p->cfg->ssid_postfix_len;
	} else {
		os_memcpy(ssid, "DIRECT-AIC-P2P-SUNPLUS", 22);
		*ssid_len = 22;
	}
	printf("p2p->cfg->ssid_postfix %s, ssid %s\n", p2p->cfg->ssid_postfix, ssid);
#endif
}
