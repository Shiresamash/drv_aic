			       ...
			       const u8 *data, size_t len, int rx_freq)
{
	...
	u16 config_methods;
	u16 allowed_config_methods = WPS_CONFIG_DISPLAY | WPS_CONFIG_KEYPAD | WPS_CONFIG_PUSHBUTTON;
	...
	if (g_aic_wifi_event_cb) {
		...
		if (msg.wfd_subelems) {
			...
			wfd_dev_info_hex = wifi_display_subelem_hex(msg.wfd_subelems,
						    WFD_SUBELEM_DEVICE_INFO);
			char dev_info_port[4];
			strncpy(dev_info_port, wfd_dev_info_hex + 4, 4);
			sscanf(dev_info_port, "%x", &port_number);
			p2p_dbg(p2p, "wfd_dev_info_hex is %s, %s, %d\n", wfd_dev_info_hex, dev_info_port, port_number);
		} else
			p2p_err(p2p, "msg.wfd_subelems is null\n");
		#endif
		aic_wifi_event_data enData = {0};
		#ifdef CONFIG_P2P
		enData.p2p_enabled = true;
		enData.p2p_dev_port_num = port_number;
		#endif
		enData.data.auth_deauth_data.reserved[0] = sa[0];
		enData.data.auth_deauth_data.reserved[1] = sa[1];
		enData.data.auth_deauth_data.reserved[2] = sa[2];
		enData.data.auth_deauth_data.reserved[3] = sa[3];
		enData.data.auth_deauth_data.reserved[4] = sa[4];
		enData.data.auth_deauth_data.reserved[5] = sa[5];
		g_aic_wifi_event_cb(PRO_DISC_REQ_EVENT, &enData);
	}
