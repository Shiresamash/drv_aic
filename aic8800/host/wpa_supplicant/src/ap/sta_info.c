			   ...
			   int authorized)
{
	...
	if (authorized) {
		...
		if (g_aic_wifi_event_cb) {
		    ...
		    uint8_t *mac_addr = (uint8_t *)&sta->addr;
			...
			if (sta->p2p_ie == NULL || sta->no_p2p_set) {
            	g_aic_wifi_event_cb(EAPOL_STA_FIN_EVENT, &enData);
			} else {
				g_aic_wifi_event_cb(EAPOL_P2P_FIN_EVENT, &enData);
			}
	    } 
		...
		if (g_aic_wifi_event_cb) {
			...
			enData.data.auth_deauth_data.reserved[5] = mac_addr[5];
			if (sta->p2p_ie == NULL || sta->no_p2p_set) {
				g_aic_wifi_event_cb(DISASSOC_STA_IND_EVENT, &enData);
			} else {
				g_aic_wifi_event_cb(DISASSOC_P2P_IND_EVENT, &enData);
			}
		}
